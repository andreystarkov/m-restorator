  ...
  Pongstgrm.prototype.template = {
    ...
    , thumb: function (options) {
        var _thumbnail  = '<div class="'+ options.dflt.column +'">'
            _thumbnail += ' <div class="thumbnail inst-thumb text-center ' + options.dflt.effects + '">'
    ...
    , bsmodal: function (options) {
        ...
        var modal  = '<div id="'+ options.data.id +'" class="modal fade">'
          ...
          navigator.userAgent.match(/webkit/i) || navigator.userAgent.match(/(iPod|iPhone|iPad)/) ?
            ...
            modal += '</div>'
            modal += '<div class="inst-box">'

            modal += '<div class="inst-comment">'
            modal += '  <a href="https://instagram.com/'+ options.data.username +'" class="media-object pull-left" style="display:none">'
            modal += '    <img src="'+ options.data.profile_picture +'" width="'+ options.dflt.picture_size +'" height="'+ options.dflt.picture_size +'" class="">'
            modal += '  </a>'
            modal += '  <div class="top-comm">'
/*            modal += '      <h5 class="top-head">'
            modal += '        <a href="https://instagram.com/'+ options.data.username +'">'+ options.data.username +'</a>'
            modal += '      </h5>'*/

          options.data.caption !== null ?
            modal += '      <p><i style="font-size:16px; padding-top: 8px;">'+ options.data.caption +'</i></p>': ""
            modal += '  </div>'
            modal += '</div>'
          ...
          options.data.comments_count !== 0 ?
            $.each(options.data.comments_data, function(a, b){
              modal += '<div class="inst-box">'
              modal += '  <a href="https://instagram.com/'+ b.from.username +'" class="media-object user-pic pull-left">'
